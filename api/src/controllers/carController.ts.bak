...
export async function create(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.CreateCarPayload } = req
...
export async function update(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.UpdateCarPayload } = req
  const { _id } = body
  ...
  try {
    ...
    if (car) {
      ...
      car.type = type as gimmekeysTypes.CarType
      car.price = price
      car.deposit = deposit
      car.seats = seats
      car.doors = doors
      car.aircon = aircon
      car.gearbox = gearbox as gimmekeysTypes.GearboxType
      car.fuelPolicy = fuelPolicy as gimmekeysTypes.FuelPolicy
      car.mileage = mileage
...
export async function getCars(req: Request, res: Response) {
  try {
    const { body }: { body: gimmekeysTypes.GetCarsPayload } = req
    ...
    if ($match.$and) {
      ...
      if (mileage) {
        if (mileage.length === 1 && mileage[0] === gimmekeysTypes.Mileage.Limited) {
          $match.$and.push({ mileage: { $gt: -1 } })
        } else if (mileage.length === 1 && mileage[0] === gimmekeysTypes.Mileage.Unlimited) {
      ...
      if (availability) {
        if (availability.length === 1 && availability[0] === gimmekeysTypes.Availablity.Available) {
          $match.$and.push({ available: true })
        } else if (availability.length === 1
          && availability[0] === gimmekeysTypes.Availablity.Unavailable) {
...
export async function getBookingCars(req: Request, res: Response) {
  try {
    const { body }: { body: gimmekeysTypes.GetBookingCarsPayload } = req
