 ...
 * @param {Request} req
 * @param {Response} res
 * @param {gimmekeysTypes.UserType} userType
 * @returns {unknown}
 */
async function _signup(req: Request, res: Response, userType: gimmekeysTypes.UserType) {
  const { body }: { body: gimmekeysTypes.SignUpPayload } = req
...
export async function signup(req: Request, res: Response) {
  await _signup(req, res, gimmekeysTypes.UserType.User)
}
...
export async function adminSignup(req: Request, res: Response) {
  await _signup(req, res, gimmekeysTypes.UserType.Admin)
}
...
export async function create(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.CreateUserPayload } = req
  ...
  try {
    ...
    const mailOptions = {
      ...
      html:
        ...
        ${strings.ACCOUNT_ACTIVATION_LINK}<br><br>
        ${Helper.joinURL(
          user.type === gimmekeysTypes.UserType.User ? env.FRONTEND_HOST : env.BACKEND_HOST,
          'activate',
...
export async function checkToken(req: Request, res: Response) {
  ...
  try {
    ...
    if (user) {
      const type = req.params.type.toLowerCase() as gimmekeysTypes.AppType

      if (
        ![gimmekeysTypes.AppType.Frontend, gimmekeysTypes.AppType.Backend].includes(type)
        || (type === gimmekeysTypes.AppType.Backend && user.type === gimmekeysTypes.UserType.User)
        || (type === gimmekeysTypes.AppType.Frontend && user.type !== gimmekeysTypes.UserType.User)
        || user.active
      ) {
...
export async function activate(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.ActivatePayload } = req
  const { userId } = body
...
export async function signin(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.SignInPayload } = req
  ...
  try {
    ...
    if (passwordMatch) {
      ...
      const loggedUser: gimmekeysTypes.User = {
        _id: user._id,
        email: user.email,
...
export async function validateEmail(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.ValidateEmailPayload } = req
  const { email } = body
...
export async function resendLink(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.ResendLinkPayload } = req
  const { email } = body
...
export async function update(req: Request, res: Response) {
  try {
    const { body }: { body: gimmekeysTypes.UpdateUserPayload } = req
    const { _id } = body
    ...
    if (type) {
      user.type = type as gimmekeysTypes.UserType
    }
...
export async function updateEmailNotifications(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.UpdateEmailNotificationsPayload } = req
...
export async function updateLanguage(req: Request, res: Response) {
  try {
    const { body }: { body: gimmekeysTypes.UpdateLanguagePayload } = req
...
export async function changePassword(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.ChangePasswordPayload } = req
  const {
    _id,
...
export async function getUsers(req: Request, res: Response) {
  try {
    ...
    const { body }: { body: gimmekeysTypes.GetUsersBody } = req
  ...
  try {
    ...
    for (const id of ids) {
      ...
      if (user) {
        ...
        if (user.type === gimmekeysTypes.UserType.Company) {
        ...
        } else if (user.type === gimmekeysTypes.UserType.User) {
