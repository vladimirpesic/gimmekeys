...
export async function validate(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.ValidateSupplierPayload } = req
  const { fullName } = body
  ...
  try {
    ...
    const user = await User.findOne({
      type: gimmekeysTypes.UserType.Company,
...
export async function update(req: Request, res: Response) {
  const { body }: { body: gimmekeysTypes.UpdateSupplierPayload } = req
  const { _id } = body
...
export async function getAllSuppliers(req: Request, res: Response) {
  try {
    let data = await User.aggregate(
      [
        { $match: { type: gimmekeysTypes.UserType.Company } },
        { $sort: { fullName: 1 } },
      ],
      { collation: { locale: env.DEFAULT_LANGUAGE, strength: 2 } },
    )
