...
interface CarListProps {
  ...
  hideCompany?: boolean
  loading?: boolean
  onLoad?: gimmekeysTypes.DataEvent<gimmekeysTypes.Car>
}
...
function CarList({
  ...
  const [rows, setRows] = useState<gimmekeysTypes.Car[]>([])
  ...
  useEffect(() => {
    if (from && to) {
      setDays(gimmekeysHelper.days(from, to))
    }
  }, [from, to])
  ...
  const _fetch = async (
    ...
    try {
      setLoading(true)
      const payload: gimmekeysTypes.GetCarsPayload = {
        ...
        {rows.length === 0
          ...
          && rows.map((car) => (
            <article key={car._id}>
              ...
              <div className="car">
                <img src={gimmekeysHelper.joinURL(Env.CDN_CARS, car.image)} alt={car.name} className="car-img" />
                {!hideCompany && (
                  <div className="car-company">
                    <span className="car-company-logo">
                      <img src={gimmekeysHelper.joinURL(Env.CDN_USERS, car.company.avatar)} alt={car.company.fullName} />
                    </span>
              ...
              {!hidePrice && (
                <div className="price">
                  ...
                  <span className="price-main">{`${gimmekeysHelper.formatNumber(Helper.price(car, from as Date, to as Date))} ${commonStrings.CURRENCY}`}</span>
