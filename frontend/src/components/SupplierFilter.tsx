...
function SupplierFilter({
  ...
  useEffect(() => {
    ...
    setCheckedSuppliers(gimmekeysHelper.flattenCompanies(companies))
  }, [companies])
  ...
  const handleUncheckAllChange = () => {
    if (allChecked) {
      ...
      const companyIds = gimmekeysHelper.flattenCompanies(suppliers)
      setAllChecked(true)
      setCheckedSuppliers(companyIds)

      if (onChange) {
        onChange(gimmekeysHelper.clone(companyIds))
      }
    }
  }
          ...
          <ul className="company-list">
            {suppliers.map((supplier, index) => (
              <li key={supplier._id}>
                ...
                >
                  <img src={gimmekeysHelper.joinURL(Env.CDN_USERS, supplier.avatar)} alt={supplier.fullName} />
                </span>
              </li>
            ))}
          </ul>
