...
interface BookingFilterProps {
  ...
  className?: string,
  language?: string,
  onSubmit?: (filter: gimmekeysTypes.Filter | null) => void
}
...
function BookingFilter({
  ...
  const handlePickupLocationChange = (locations: gimmekeysTypes.Option[]) => {
    setPickupLocation(locations.length > 0 ? locations[0]._id : '')
  }

  const handleDropOffLocationChange = (locations: gimmekeysTypes.Option[]) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement> | React.KeyboardEvent<HTMLElement>) => {
    ...
    let filter: gimmekeysTypes.Filter | null = {
      from,
      to,
      pickupLocation,
      dropOffLocation,
    ...
    if (onSubmit) {
      onSubmit(gimmekeysHelper.clone(filter))
    }
  }
