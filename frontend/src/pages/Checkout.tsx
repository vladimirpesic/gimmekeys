...
function Checkout() {
  ...
  const [pickupLocation, setPickupLocation] = useState<gimmekeysTypes.Location>()
  const [dropOffLocation, setDropOffLocation] = useState<gimmekeysTypes.Location>()
  ...
  const handleCancellationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (car && from && to) {
      ...
      const options: gimmekeysTypes.CarOptions = {
  ...
  const validateBirthDate = (date?: Date) => {
    if (car && date && gimmekeysHelper.isDate(date)) {
      const now = new Date()
  ...
  const validateCardMonth = (_cardMonth?: string) => {
    if (_cardMonth) {
      if (gimmekeysHelper.isInteger(_cardMonth)) {
  ...
  const validateCardYear = (_cardYear?: string) => {
    if (_cardYear) {
      if (gimmekeysHelper.isYear(_cardYear)) {
  ...
  const validateCvv = (_cvv?: string) => {
    if (_cvv) {
      const _cvvValid = gimmekeysHelper.isCvv(_cvv)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      let driver: gimmekeysTypes.User | undefined
      let _additionalDriver: gimmekeysTypes.AdditionalDriver | undefined
      ...
      const booking: gimmekeysTypes.Booking = {
        ...
        from,
        to,
        status: payLater ? gimmekeysTypes.BookingStatus.Pending : gimmekeysTypes.BookingStatus.Paid,
        cancellation,
        amendments,
        theftProtection,
        collisionDamageWaiver,
      ...
      const payload: gimmekeysTypes.BookPayload = {
        driver,
        booking,
        additionalDriver: _additionalDriver,
        payLater,
  ...
  const onLoad = async (_user?: gimmekeysTypes.User) => {
    ...
    if (params.has('f')) {
      ...
      _from = val && gimmekeysHelper.isInteger(val) && new Date(Number.parseInt(val, 10))
    }
    if (params.has('t')) {
      const val = params.get('t')
      _to = val && gimmekeysHelper.isInteger(val) && new Date(Number.parseInt(val, 10))
    }
                ...
                <div className="booking-details-container">
                  ...
                  <div className="booking-details">
                    ...
                    <div className="booking-detail" style={{ height: bookingDetailHeight }}>
                      ...
                      <div className="booking-detail-value">
                        <div className="car-company">
                          <img src={gimmekeysHelper.joinURL(Env.CDN_USERS, car.company.avatar)} alt={car.company.fullName} style={{ height: Env.COMPANY_IMAGE_HEIGHT }} />
                    ...
                    <div className="booking-detail" style={{ height: bookingDetailHeight }}>
                      ...
                      <div className="booking-detail-value booking-price">{`${gimmekeysHelper.formatNumber(price)} ${commonStrings.CURRENCY}`}</div>
                    </div>
                  </div>
                </div>
                {!authenticated && (
                  ...
                  <div className="payment">
                    <div className="cost">
                      ...
                      <div className="secure-payment-cost">
                        ...
                        <span className="cost-value">{`${gimmekeysHelper.formatNumber(price)} ${commonStrings.CURRENCY}`}</span>
                      </div>
                    </div>
