...
function SupplierSelectList({
  ...
  const [rows, setRows] = useState<gimmekeysTypes.Option[]>([])
  ...
  useEffect(() => {
    ...
    if (value && !gimmekeysHelper.arrayEqual(selectedOptions, _value)) {
      setSelectedOptions(_value as gimmekeysTypes.Option[])
    }
  }, [value, multiple, selectedOptions])

  const getCompanies = (companies: gimmekeysTypes.User[]): gimmekeysTypes.Option[] =>
