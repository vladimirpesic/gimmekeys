  ...
  return (
    <div>
      ...
      <Select label={label} value={value} onChange={handleChange} variant={variant || 'standard'} required={required} fullWidth>
        <MenuItem value={gimmekeysTypes.GearboxType.Automatic}>{strings.GEARBOX_AUTOMATIC}</MenuItem>
        <MenuItem value={gimmekeysTypes.GearboxType.Manual}>{strings.GEARBOX_MANUAL}</MenuItem>
      </Select>
    </div>
  )
}
