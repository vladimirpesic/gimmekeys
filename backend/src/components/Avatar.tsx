...
interface AvatarProps {
  ...
  mode?: 'create' | 'update',
  type?: string,
  record?: gimmekeysTypes.User | gimmekeysTypes.Car | null,
...
function Avatar({
  ...
  const [avatarRecord, setAvatarRecord] = useState<gimmekeysTypes.User | gimmekeysTypes.Car>()
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      if (type === gimmekeysTypes.RecordType.Admin
        || type === gimmekeysTypes.RecordType.Company
      ...
      } else if (type === gimmekeysTypes.RecordType.Car) {
        if (mode === 'create') {
          const createAvatar = async () => {
            try {
  ...
  const handleDelete = async () => {
    try {
      if (type === gimmekeysTypes.RecordType.Admin || type === gimmekeysTypes.RecordType.Company || type === gimmekeysTypes.RecordType.User) {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      if (record) {
        ...
        if (type === gimmekeysTypes.RecordType.Car) {
          setAvatar((record as gimmekeysTypes.Car).image)
        } else {
  ...
  const userAvatar = avatar ? <MaterialAvatar src={gimmekeysHelper.joinURL(cdn(), avatar)} className={size ? `avatar-${size}` : 'avatar'} /> : <></>
  ...
  return !error && !loading ? (
    <div className={className}>
      {avatar ? (
        readonly ? (
          type === gimmekeysTypes.RecordType.Car ? (
            <img style={carImageStyle} src={gimmekeysHelper.joinURL(cdn(), avatar)} alt={avatarRecord && (avatarRecord as gimmekeysTypes.Car).name} />
          ) : type === gimmekeysTypes.RecordType.Company ? (
            <div className="company-avatar-readonly">
              <img src={gimmekeysHelper.joinURL(cdn(), avatar)} alt={avatarRecord && avatarRecord.fullName} />
            </div>
          ...
          >
            <Badge
              ...
              className={type === gimmekeysTypes.RecordType.Company ? 'company-avatar' : ''}
              badgeContent={(
            ...
            >
              {type === gimmekeysTypes.RecordType.Car ? (
                <div className="car-avatar">
                  <img src={gimmekeysHelper.joinURL(cdn(), avatar)} alt={avatarRecord && (avatarRecord as gimmekeysTypes.Car).name} />
                </div>
              ) : type === gimmekeysTypes.RecordType.Company ? (
                <img style={companyImageStyle} src={gimmekeysHelper.joinURL(cdn(), avatar)} alt={avatarRecord && avatarRecord.fullName} />
              ) : (
                <MaterialAvatar src={gimmekeysHelper.joinURL(cdn(), avatar)} className={size ? `avatar-${size}` : 'avatar'} />
              )}
            </Badge>
          </Badge>
        )
