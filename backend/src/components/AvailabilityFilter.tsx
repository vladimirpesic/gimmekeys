...
function AvailabilityFilter({
  ...
  const [values, setValues] = useState<string[]>([
    gimmekeysTypes.Availablity.Available,
    gimmekeysTypes.Availablity.Unavailable
  ])
  ...
  const handleAvailableChange = (e: React.ChangeEvent<HTMLInputElement> | React.MouseEvent<HTMLElement>) => {
    if (e.currentTarget instanceof HTMLInputElement) {
      if (e.currentTarget.checked) {
        values.push(gimmekeysTypes.Availablity.Available)
        ...
        values.splice(
          values.findIndex((v) => v === gimmekeysTypes.Availablity.Available),
          1,
        )
  ...
  const handleUnavailableChange = (e: React.ChangeEvent<HTMLInputElement> | React.MouseEvent<HTMLElement>) => {
    if (e.currentTarget instanceof HTMLInputElement) {
      if (e.currentTarget.checked) {
        values.push(gimmekeysTypes.Availablity.Unavailable)
        ...
        values.splice(
          values.findIndex((v) => v === gimmekeysTypes.Availablity.Unavailable),
          1,
        )
  ...
  const handleUncheckAllChange = () => {
    if (availableRef.current && unavailableRef.current) {
      if (allChecked) {
        ...
        const _values = [gimmekeysTypes.Availablity.Available, gimmekeysTypes.Availablity.Unavailable]
