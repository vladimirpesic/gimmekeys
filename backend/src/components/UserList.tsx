...
interface UserListProps {
  types?: gimmekeysTypes.UserType[]
  keyword?: string
  user?: gimmekeysTypes.User
  hideDesktopColumns?: boolean
  checkboxSelection?: boolean
  onLoad?: gimmekeysTypes.DataEvent<gimmekeysTypes.User>
}
...
function UserList({
  ...
  const [columns, setColumns] = useState<GridColDef<gimmekeysTypes.User>[]>([])
  const [rows, setRows] = useState<gimmekeysTypes.User[]>([])
  ...
  const _fetch = async (_page: number, _user?: gimmekeysTypes.User) => {
    try {
      if (_user && types) {
        ...
        const payload: gimmekeysTypes.GetUsersBody = {
  ...
  const getColumns = (_user: gimmekeysTypes.User): GridColDef<gimmekeysTypes.User>[] => {
    const _columns: GridColDef<gimmekeysTypes.User>[] = [
      {
        field: 'fullName',
        headerName: commonStrings.USER,
        flex: 1,
        renderCell: ({ row, value }: GridRenderCellParams<gimmekeysTypes.User, string>) => {
          const __user = row
          let userAvatar

          if (__user.avatar) {
            if (__user.type === gimmekeysTypes.RecordType.Company) {
              userAvatar = <img src={gimmekeysHelper.joinURL(Env.CDN_USERS, row.avatar)} alt={row.fullName} />
            } else {
              const avatar = <Avatar src={gimmekeysHelper.joinURL(Env.CDN_USERS, row.avatar)} className="avatar-small" />
              if (__user.verified) {
                userAvatar = (
                  <Badge
                    overlap="circular"
      ...
      },
      {
        ...
        flex: 1,
        valueGetter: ({ value }: GridValueGetterParams<gimmekeysTypes.User, string>) => value,
      },
      {
        field: 'phone',
      ...
      },
      {
        ...
        flex: 1,
        renderCell: ({ value }: GridRenderCellParams<gimmekeysTypes.User, gimmekeysTypes.UserType>) => <span className={`bs us-${value?.toLowerCase()}`}>{Helper.getUserType(value)}</span>,
        valueGetter: ({ value }: GridValueGetterParams<gimmekeysTypes.User, string>) => value,
      },
      {
        field: 'action',
        ...
        renderCell: ({ row }: GridRenderCellParams<gimmekeysTypes.User>) => {
          ...
          const __user = row
          return _user.type === gimmekeysTypes.RecordType.Admin || __user.company === _user._id ? (
            <div>
