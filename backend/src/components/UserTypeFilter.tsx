...
interface UserTypeFilterProps {
  className?: string,
  onChange?: (types: gimmekeysTypes.UserType[]) => void
}
...
function UserTypeFilter({
  ...
  const [checkedUserTypes, setCheckedUserTypes] = useState<gimmekeysTypes.UserType[]>(userTypes.map((user) => user.value))
  ...
  const handleUserTypeChange = (e: React.ChangeEvent<HTMLInputElement> | React.MouseEvent<HTMLElement>) => {
    const userType = e.currentTarget.getAttribute('data-value') as gimmekeysTypes.UserType
    ...
    if (onChange) {
      onChange(gimmekeysHelper.clone(checkedUserTypes))
    }
  }
  ...
  const handleUncheckAllChange = () => {
    if (allChecked) {
      ...
      if (onChange) {
        onChange(gimmekeysHelper.clone(_userTypes))
      }
    }
  }
