...
function CreateBooking() {
  ...
  const handleCarSelectListChange = useCallback((values: gimmekeysTypes.Car[]) => {
  ...
  const _validateBirthDate = (date?: Date) => {
    if (date && gimmekeysHelper.isDate(date)) {
      const now = new Date()
  ...
  const onLoad = (user?: gimmekeysTypes.User) => {
    if (user) {
      setVisible(true)

      if (user.type === gimmekeysTypes.RecordType.Company) {
