...
function UpdateBooking() {
  ...
  const handleDriverChange = (values: gimmekeysTypes.Option[]) => {
  ...
  const handleCarSelectListChange = useCallback(
    async (values: gimmekeysTypes.Car[]) => {
      try {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      let payload: gimmekeysTypes.UpsertBookingPayload
      let _additionalDriver: gimmekeysTypes.AdditionalDriver
      if (additionalDriverSet) {
      ...
      if (params.has('b')) {
        ...
        if (id && id !== '') {
          try {
            ...
            if (_booking) {
              if (!Helper.admin(_user) && (_booking.company as gimmekeysTypes.User)._id !== _user._id) {
                setLoading(false)
                setNoMatch(true)
                return
              }
              ...
              setLoading(false)
              setVisible(true)
              setIsCompany(_user.type === gimmekeysTypes.RecordType.Company)
              ...
              if (_booking.additionalDriver && _booking._additionalDriver) {
                const _additionalDriver = _booking._additionalDriver as gimmekeysTypes.AdditionalDriver
          ...
          <div className="col-2">
            <div className="col-2-header">
              <div className="price">
                ...
                <span className="price-main">{`${gimmekeysHelper.formatNumber(price ?? 0)} ${commonStrings.CURRENCY}`}</span>
            ...
            <CarList
              ...
              user={user}
              booking={booking}
              cars={((car && [booking.car]) as gimmekeysTypes.Car[]) || []}
              hidePrice
            />
          </div>
