...
function Car() {
    ...
    if (_user && _user.verified && params.has('cr')) {
      ...
      if (id && id !== '') {
        try {
          ...
          if (_car) {
            if (_user.type === gimmekeysTypes.RecordType.Admin) {
              try {
                const _companies = await SupplierService.getAllSuppliers()
                const companyIds = gimmekeysHelper.flattenCompanies(_companies)
  ...
  const edit = user && car && car.company && (user.type === gimmekeysTypes.RecordType.Admin || user._id === car.company._id)
        ...
        <div className="car">
          <div className="col-1">
            <section className="car-sec">
              ...
              <div className="price">{`${gimmekeysHelper.formatNumber(car.price)} ${strings.CAR_CURRENCY}`}</div>
