...
function UpdateUser() {
  ...
  const handleUserTypeChange = async (e: SelectChangeEvent<string>) => {
    ...
    if (_type === gimmekeysTypes.RecordType.Company) {
  ...
  const validateBirthDate = (date?: Date) => {
    if (date && gimmekeysHelper.isDate(date) && type === gimmekeysTypes.RecordType.User) {
      const now = new Date()
  ...
  const onAvatarChange = (_avatar: string) => {
    if (loggedUser && user && loggedUser._id === user._id) {
      const _loggedUser = gimmekeysHelper.clone(loggedUser)
      _loggedUser.avatar = _avatar

      setLoggedUser(_loggedUser)
    }

    const _user = gimmekeysHelper.clone(user)
    ...
    if (_avatar !== null && type === gimmekeysTypes.RecordType.Company) {
      setAvatarError(false)
    }
  }
  ...
  const handleResendActivationLink = async () => {
    try {
      const status = await UserService.resend(email, false, type === gimmekeysTypes.RecordType.User ? 'frontend' : 'backend')
  ...
  const onLoad = async (_loggedUser?: gimmekeysTypes.User) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      if (type === gimmekeysTypes.RecordType.Company && !avatar) {
        setAvatarError(true)
        setError(false)
        return
      }
      ...
      const data: gimmekeysTypes.UpdateUserPayload = {
  ...
  const company = type === gimmekeysTypes.RecordType.Company
  const driver = type === gimmekeysTypes.RecordType.User
  const activate = admin
    || (loggedUser && user && loggedUser.type === gimmekeysTypes.RecordType.Company && user.type === gimmekeysTypes.RecordType.User && user.company as string === loggedUser._id)
          ...
          <Paper className="user-form user-form-wrapper" elevation={10}>
            ...
            <form onSubmit={handleSubmit}>
              <Avatar
                ...
                hideDelete={type === gimmekeysTypes.RecordType.Company}
              />
              ...
              {admin && (
                <FormControl fullWidth margin="dense" style={{ marginTop: company ? 0 : 39 }}>
                  ...
                  <Select label={commonStrings.TYPE} value={type} onChange={handleUserTypeChange} variant="standard" required fullWidth>
                    <MenuItem value={gimmekeysTypes.RecordType.Admin}>{Helper.getUserType(gimmekeysTypes.UserType.Admin)}</MenuItem>
                    <MenuItem value={gimmekeysTypes.RecordType.Company}>{Helper.getUserType(gimmekeysTypes.UserType.Company)}</MenuItem>
                    <MenuItem value={gimmekeysTypes.RecordType.User}>{Helper.getUserType(gimmekeysTypes.UserType.User)}</MenuItem>
                  </Select>
                </FormControl>
              )}
