...
function UpdateCar() {
  ...
  const [locations, setLocations] = useState<gimmekeysTypes.Option[]>([])
  ...
  const handleLocationsChange = (_locations: gimmekeysTypes.Option[]) => {
      ...
      if (params.has('cr')) {
        ...
        if (id && id !== '') {
          try {
            ...
            if (_car) {
              if (_user.type === gimmekeysTypes.RecordType.Company && _user._id !== _car.company._id) {
                setLoading(false)
                setNoMatch(true)
                return
              }
              ...
              const lcs: gimmekeysTypes.Option[] = []
              for (const loc of _car.locations) {
                const { _id, name: _name } = loc
                const lc: gimmekeysTypes.Option = { _id, name: _name ?? '' }
                lcs.push(lc)
              }
              setLocations(lcs)
  ...
  const admin = user && user.type === gimmekeysTypes.RecordType.Admin
