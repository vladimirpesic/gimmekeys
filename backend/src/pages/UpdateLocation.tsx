...
function UpdateLocation() {
  ...
  const [names, setNames] = useState<gimmekeysTypes.LocationName[]>([])
  const [nameErrors, setNameErrors] = useState<boolean[]>([])
  const [noMatch, setNoMatch] = useState(false)
  const [error, setError] = useState(false)
  const [location, setLocation] = useState<gimmekeysTypes.Location>()
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setNameErrors(gimmekeysHelper.cloneArray(nameErrors) as boolean[])
      ...
      if (isValid) {
        ...
        if (status === 200) {
          ...
          setLocation(gimmekeysHelper.clone(location))
        ...
        if (id && id !== '') {
          try {
            ...
            if (_location && _location.values) {
              ...
              const _names: gimmekeysTypes.LocationName[] = _location.values.map((value) => ({
                ...
                <FormControl key={value.value} fullWidth margin="dense">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      checkName()
                      setNames(gimmekeysHelper.cloneArray(names) as gimmekeysTypes.LocationName[])
                    }}
                    autoComplete="off"
                  />
