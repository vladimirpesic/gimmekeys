...
function NotificationsScreen({ navigation, route }: NativeStackScreenProps<StackParams, 'Notifications'>) {
  ...
  const [rows, setRows] = useState<gimmekeysTypes.Notification[]>([])
  const [totalRecords, setTotalRecords] = useState(-1)
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false)
  const [selectedRows, setSelectedRows] = useState<gimmekeysTypes.Notification[]>([])
                ...
                {rows.map((row) => (
                  <View key={row._id} style={styles.notificationContainer}>
                    ...
                    <View style={styles.notification}>
                      ...
                      >
                        {gimmekeysHelper.capitalize(format(new Date(row.createdAt as Date), _format, { locale }))}
                      </Text>
