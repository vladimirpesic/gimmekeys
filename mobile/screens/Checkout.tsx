...
function CheckoutScreen({ navigation, route }: NativeStackScreenProps<StackParams, 'Checkout'>) {
  ...
  const [user, setUser] = useState<gimmekeysTypes.User | null>()
  ...
  const [car, setCar] = useState<gimmekeysTypes.Car | null>(null)
  const [pickupLocation, setPickupLocation] = useState<gimmekeysTypes.Location | null>(null)
  const [dropOffLocation, setDropOffLocation] = useState<gimmekeysTypes.Location | null>(null)
  ...
  const _init = async () => {
    try {
      ...
      let _user: gimmekeysTypes.User | null = null
  ...
  const validateCvv = () => {
    if (cvv) {
      const _cvvValid = gimmekeysHelper.isCvv(cvv)
      setCardCvvRequired(false)
  ...
  const onCancellationChange = (checked: boolean) => {
    ...
    const _price = Helper.price(car as gimmekeysTypes.Car, from as Date, to as Date, options)
              ...
              <View style={styles.contentContainer}>
                ...
                <View style={styles.section}>
                  ...
                  <Text style={styles.detailText}>
                    {`${Helper.getDaysShort(gimmekeysHelper.days(from, to))} (${gimmekeysHelper.capitalize(format(from, _format, { locale }))} - ${gimmekeysHelper.capitalize(
                  ...
                  <View style={styles.company}>
                    <Image
                      ...
                      source={{
                        uri: gimmekeysHelper.joinURL(Env.CDN_USERS, car.company.avatar),
                      }}
                    />
                  ...
                  <Text style={styles.detailTextBold}>{`${gimmekeysHelper.formatNumber(price)} ${i18n.t('CURRENCY')}`}</Text>
                </View>
                  ...
                  <View style={styles.payment}>
                    <View style={styles.paymentHeader}>
                      ...
                      <View style={styles.securePaymentInfo}>
                        ...
                        <Text style={styles.costText}>{`${gimmekeysHelper.formatNumber(price)} ${i18n.t('CURRENCY')}`}</Text>
                      </View>
                    </View>
