...
interface StatusFilterProps {
  visible?: boolean
  style?: object
  onLoad?: (checkedStatuses: gimmekeysTypes.BookingStatus[]) => void
  onChange?: (checkedStatuses: gimmekeysTypes.BookingStatus[]) => void
}
...
function StatusFilter({
  ...
  const [statuses, setStatuses] = useState<gimmekeysTypes.StatusFilterItem[]>(
        ...
        <Accordion style={styles.accordion} title={i18n.t('BOOKING_STATUS')}>
          <View style={styles.statuses}>
                ...
                <View key={status.value} style={styles.status}>
                  <Switch
                    ...
                    onValueChange={(checked) => {
                      if (checked) {
                        status.checked = true
                        setStatuses(gimmekeysHelper.clone(statuses))
                      ...
                      if (onChange) {
                        onChange(gimmekeysHelper.clone(checkedStatuses))
                      }
                    }}
                  >
            ...
            <Link
              ...
              onPress={() => {
                let _checkedStatuses: gimmekeysTypes.BookingStatus[] = []
                if (allChecked) {
                  ...
                  setAllChecked(true)
                  setStatuses(gimmekeysHelper.clone(statuses))
                  _checkedStatuses = gimmekeysHelper.clone(statuses.map((status) => status.value))
                  setCheckedStatuses(_checkedStatuses)
