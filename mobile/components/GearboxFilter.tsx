...
interface GearboxFilterProps {
  visible?: boolean
  style?: object
  onChange?: (values: gimmekeysTypes.GearboxType[]) => void
}
...
function GearboxFilter({
  ...
  const [values, setValues] = useState([gimmekeysTypes.GearboxType.Automatic, gimmekeysTypes.GearboxType.Manual])
  const [allChecked, setAllChecked] = useState(true)

  const onValueChangeAutomatic = (checked: boolean) => {
    if (checked) {
      values.push(gimmekeysTypes.GearboxType.Automatic)
      ...
      values.splice(
        values.findIndex((v) => v === gimmekeysTypes.GearboxType.Automatic),
        1,
      )
  ...
  const onValueChangeManual = (checked: boolean) => {
    if (checked) {
      values.push(gimmekeysTypes.GearboxType.Manual)
      ...
      values.splice(
        values.findIndex((v) => v === gimmekeysTypes.GearboxType.Manual),
        1,
      )
          ...
          <Link
            ...
            onPress={() => {
              if (allChecked) {
                ...
                const _values = [gimmekeysTypes.GearboxType.Automatic, gimmekeysTypes.GearboxType.Manual]
                setAutomatic(true)
                setManual(true)
                setAllChecked(true)
                setValues(_values)
