...
function Booking({
  ...
  const car = booking.car as gimmekeysTypes.Car
  const supplier = booking.company as gimmekeysTypes.User
  ...
  return (
    <View key={booking._id} style={styles.bookingContainer}>
      <View style={styles.booking}>
        ...
        <Text style={styles.detailText}>{(booking.pickupLocation as gimmekeysTypes.Location).name}</Text>

        <Text style={styles.detailTitle}>{i18n.t('DROP_OFF_LOCATION')}</Text>
        <Text style={styles.detailText}>{(booking.dropOffLocation as gimmekeysTypes.Location).name}</Text>

        <Text style={styles.detailTitle}>{i18n.t('CAR')}</Text>
        <Text style={styles.detailText}>{`${car.name} (${gimmekeysHelper.formatNumber(car.price)} ${i18n.t('CAR_CURRENCY')})`}</Text>
        ...
        <Text style={styles.detailTextBold}>{`${gimmekeysHelper.formatNumber(booking.price)} ${i18n.t('CURRENCY')}`}</Text>

        {booking.cancellation
          && !booking.cancelRequest
          && booking.status !== gimmekeysTypes.BookingStatus.Cancelled
